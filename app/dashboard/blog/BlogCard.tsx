  ...
  return (
    <div className='min-h-[60vh] gap-5 w-full flex flex-wrap justify-between items-start'>
      <div className='gap-5 w-full flex flex-wrap justify-between'>
        {sortByNewest && blogs.sort((b1: BlogType, b2: BlogType) => b2.createdAt - b1.createdAt).map((blog: BlogType, index: number) => (
        ...
        {!sortByNewest && blogs.sort((b1: BlogType, b2: BlogType) => b1.createdAt - b2.createdAt).map((blog: BlogType, index: number) => (
          <div key={index} className='w-64 h-[100px] shadow-lg bg-white hover:scale-110 duration-300 rounded-xl blog-card flex overflow-hidden'>
            <Image className='h-full w-1/3 object-cover' src={blog.image} alt={blog.title.slice(0, 20)} width={200} height={200} priority />
            <div className='h-full flex-1 flex flex-col justify-between p-4 gap-2'>
              <div className='text-sm text-black font-bold'>{blog.title.slice(0, 30)}</div>
