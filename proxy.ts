...
export async function proxy(request: NextRequest) {
  ...
  const securedPostRoutes = ["/api/administrator", "/api/news"];
  const securedGetRoutes = ["/api/administrator/me"];
  const securedPutRoutes = ["/api/news"];
  const securedDeleteRoutes = ["/api/news"];
